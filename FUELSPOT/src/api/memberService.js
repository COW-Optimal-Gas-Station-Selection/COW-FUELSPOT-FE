...
export const sendVerificationCode = async (emailRequest) => {
  try {
    ...
    throw new Error(error.response?.data?.message || '인증 코드 발송 중 오류가 발생했습니다.');
  }
};
...
export const verifyCode = async (verificationRequest) => {
  try {
    ...
    throw new Error(error.response?.data?.message || '인증 코드 검증 중 오류가 발생했습니다.');
  }
};
...
export const resetPassword = async (resetRequest) => {
  try {
    ...
    throw new Error(error.response?.data?.message || '비밀번호 재설정 중 오류가 발생했습니다.');
  }
};
