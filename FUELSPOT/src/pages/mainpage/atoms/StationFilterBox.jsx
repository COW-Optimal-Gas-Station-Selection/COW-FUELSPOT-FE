    ...
    <div className="flex items-center gap-2">
      <Listbox value={sortType} onChange={onSortChange}>
        <div className="relative min-w-[180px]">
          <Listbox.Button className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm bg-white shadow-sm flex items-center justify-between hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-200 cursor-pointer">
            <span className="flex items-center gap-2 min-w-0">
              <span className="truncate max-w-[110px] text-neutral-950 font-medium">{sortOptions.find(opt => opt.value === sortType)?.label}</span>
              ...
              {sortOptions.map(opt => (
                <Listbox.Option
                  ...
                  className={({ active, selected }) =>
                    `px-3 py-2.5 rounded-md cursor-pointer flex items-center gap-2 text-sm transition-all duration-150 ${active ? 'bg-blue-50' : 'bg-white'} ${selected ? 'font-semibold text-blue-600 bg-blue-50' : 'text-neutral-950'} hover:bg-blue-50`
                  }
                >
