...
const StationCard = ({ station, onClick, onNavigate, isFavorite, onToggleFavorite, className = '', ...props }) => {
  return (
    <div 
      className={`relative border-b border-[#f3f4f6] p-4 flex flex-col gap-3 hover:bg-gray-50 transition-colors cursor-pointer ${className}`} 
      onClick={onClick} 
      {...props}
    >
      ...
      {onToggleFavorite && (
        <div className="absolute top-4 right-4 z-[1] flex flex-col items-center">
          <FavoriteButton
            stationId={station.id}
            isFavorite={isFavorite}
            onToggle={onToggleFavorite}
          />
          <span className="text-[10px] text-gray-400 bg-gray-50/80 rounded px-1.5 py-0.5 border border-gray-100 mt-0.5 whitespace-nowrap">
            {mockFavoriteCounts[station.id] || 0}명
          </span>
        </div>
      )}
